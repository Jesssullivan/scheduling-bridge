...
- the `runFlow` fold — the only async-job execution path (the three legacy
  hand-written compositions were deleted in the 0.7.0 deletion gate) — emits
  per-stepId Prometheus metrics (`acuity_flow_step_attempts_total`,
  `acuity_flow_step_failures_total`, `acuity_flow_step_landings_total`, and the
  step-duration histogram), bounded by the registered plan step ids
...
- out-of-ecosystem (npm-style) consumers of the GitHub Packages artifact must
  ...
  {
    "dependencies": {
      "@tummycrypt/scheduling-bridge": "npm:@jesssullivan/scheduling-bridge@^0.7.1",
      "@tummycrypt/scheduling-kit": "npm:@jesssullivan/scheduling-kit@^0.9.2"
    }
  }
  ```
